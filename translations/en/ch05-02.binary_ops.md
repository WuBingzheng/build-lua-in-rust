...
For simplicity, the other parts are simplified to `OTHERS` to get:

```
exp ::= exp binop exp | OTHERS
```

It is a left recursion rule, you need to [eliminate left recursion] (./ch04-05.table_rw_and_bnf.md#eliminate left recursion) according to the method introduced before, and get:
...
Corresponding to the `exp()` function code above, the `OTHERS` part at the beginning reads the first operand `a`; then reads the operator `+` in the `while` loop; and then calls recursively The `exp()` function reads the right operand, so it needs to be calculated at this time. Also take the above two sentences as an example:

- `a + b - c`, end after reading `b` and use it as the right operand; then perform addition `a + b`; and then cycle through the following `- c` part again;
...
So in grammatical analysis, how to judge which of the above situations is the case? After reading `b`, should we stop parsing and calculate addition first, or continue parsing? It depends on the precedence of the next operator and the current operator:
...
This function appears to be a property of the `Token` type, so it seems to be a suitable method defined as `Token`. But `Token` type is defined in `lex.rs`; while priority is a concept of parsing, it should be implemented in `parse.rs`. The Rust language does not allow methods to be added to a type's non-defining file. So the above function is defined as an ordinary function in the `parse.rs` file (rather than the method of `ParseProto` like other functions).
...
The above parsing code combines loops and recursive calls, which is very difficult for those who are not familiar with the algorithm, and it is difficult to write the complete code directly. However, according to the BNF specification after eliminating left recursion, the loop and recursion can be completed, and then the function can be easily completed according to the priority and conditional exit.
...
2. Then parse the right operand `f(t)*2`, and execute f(t) during the parsing process, thus modifying the value of t.k;
...
First of all, although binary operators are mostly numerical calculations, because Lua's metatable function is similar to operator overloading, other types of constants (such as strings, bools, etc.) may be legal operands. When parsing unary operations, these types of constants will directly report an error, but for binary operations, it needs to be executed at the execution stage to determine whether it is legal.
...
Otherwise, bytecode is generated and executed by the virtual machine. Similar to [Read Global Variables](./ch02-00.variables.md) and [Read Table](./ch04-05.table_rw_and_bnf.md) operations that have been supported before, each binary operator is also set to 3 Each bytecode handles three types of right operands: variables on the stack, constants, and small integers.

The left operand is uniformly discharged to the stack, because it is rare for the left operand to be a constant. If you also add corresponding bytecodes for constants and small integer types, such as `10-a`, then there are too many bytecode types.

Finally, for addition and multiplication that satisfy the commutative law, if the left operation is a constant, then it can be exchanged. For example, `10+a` can be converted to `a+10` first. Since the right operand `10` is a small integer, it can be Use `AddInt` bytecode.
...
If it is not a constant and cannot be folded, then call the `do_binop()` function to return ExpDesc. Here, the enum tag is used as a function, which has been introduced [before](./ch04-04.expdesc_rewrite.md#tableconstructor), and will not be introduced here.
