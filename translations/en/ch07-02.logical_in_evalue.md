...
The previous section introduced the application of logical operations in conditional judgment. This section introduces another application scenario, that is, the processing during evaluation.

In the previous section, the grammatical analysis process of logical operations in **conditional judgment** scenarios can be divided into two parts:
...
+---------------+ ExpDesc::Test | +------------+
| (a) Processing logical operations |------------------>+
+---------------+ | +------------+
                                      \--->| (c) Evaluation |
                                           +---------+
```
...
Will output `1`, because the `&&` operator will first convert the two operands to Boolean type (both are true in this example), and then execute the `&&` operation, the result is true, which is 1 in C language . However, the Rust language is stricter, and both operands of `&&` must be of Boolean type, so the result is also of Boolean type.

But logical operations in Lua evaluate to the last **evaluated** operand. For example, the following are very common usages:

- `print(t and t.k)`, first judge whether t exists, and then find the index of t. If t does not exist, then there is no need to judge t.k, so the result is t is nil; otherwise, it is t.k.
...
The process of summarizing and summarizing based on these 4 figures is omitted here, and the evaluation rules are directly given:
...
Our approach is to define recursively. Change the first parameter type of `ExpDesc::Test` from `usize` to `ExpDesc`. Of course, it cannot be defined directly, but it needs to [encapsulate a layer of Box pointer](https://kaisery.github.io/trpl-zh-cn/ch15-01-box.html#box-%E5%85%81%E8 %AE%B8%E5%88%9B%E5%BB%BA%E9%80%92%E5%BD%92%E7%B1%BB%E5%9E%8B):
...
The function of `TestAndSetJump` is: if the value of the stack index of the first parameter is tested to be true, it is assigned to the stack position of the second parameter and jumps to the bytecode position of the third parameter. Similar to `TestOrSetJump`.
...
After introducing the above modification points, now start the syntax analysis. The so-called evaluation is discharge. So you only need to complete the `ExpDesc::Test` type in the `discharge()` function. In the previous section, this is not complete. The specific discharge method is: first discharge the recursively defined conditional statement, and then repair the judgment bytecodes in the two jump lists.
     ...
     fn discharge(&mut self, dst: usize, desc: ExpDesc) {
         let code = match desc {
             ...
             ExpDesc::Test(condition, true_list, false_list) => {
                 ...
                 self.discharge(dst, *condition); // first discharge the recursively defined conditional statement
                 self.fix_test_set_list(true_list, dst); // Fix the judgment bytecode in the True jump list
                 self.fix_test_set_list(false_list, dst); // Fix the judgment bytecode in the False jump list
                 return;
             }
```
         ...
         for i in list.into_iter() {
             ...
             let code = match self. byte_codes[i] {
                 ByteCode::TestOrJump(icondition, 0) =>
                     if icondition == dst { // If the conditional statement is just at the target position, there is no need to change it to TestAndSetJump
                         ByteCode::TestOrJump(icondition, jmp as i16)
                     } else { // Modify to TestAndSetJump bytecode
